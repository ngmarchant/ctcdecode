...
PathTrie::PathTrie(size_t group) {
...
PathTrie *PathTrie::get_path_trie(int new_char, int new_timestep, float cur_log_prob_c, bool reset) {
  std::vector<std::pair<int, PathTrie *>>::iterator child;
  ...
  if (child != children_.end()) {
    ...
    if (has_dictionary_) {
      ...
      if (!found) {
        ...
        auto *new_path = new PathTrie(group);
        new_path->character = new_char;
        new_path->timestep = new_timestep;
        new_path->length = length + 1;
...
void PathTrie::set_dictionary(fst::StdVectorFst *dictionary) {
  dictionary_ = dictionary->Copy(true);
  dictionary_state_ = dictionary_->Start();
  matcher_ = std::make_shared<FSTMATCH>(*dictionary_, fst::MATCH_INPUT);
  has_dictionary_ = true;
}

std::string PathTrie::to_string() {
  if (is_empty()) return "";
  if (character == parent->character) return parent->to_string();
  return parent->to_string() + (char) character;
}
