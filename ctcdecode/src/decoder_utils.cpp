...
std::vector<std::pair<double, Output>> get_beam_search_result(
  ...
  for (size_t i = 0; i < beam_size && i < space_prefixes.size(); ++i) {
    ...
    // The minus sign: higher score should be better, so output -log(P)
...
/**
 * Hamming distance, aligned to the end of the string
 */
size_t hamming_distance_tail(PathTrie *x, PathTrie *y) {
  if (x->is_empty() || y->is_empty()) return 0;
  return (x->character != y->character) + hamming_distance_tail(x->parent, y->parent);
}

/**
 * Hamming distance, aligned to the beginning of the string
 */
size_t hamming_distance(PathTrie *x, PathTrie *y) {
  while (x->length > y->length) x = x->parent;
  while (y->length > x->length) y = y->parent;
  return hamming_distance_tail(x, y);
}
